...
invoice_mcp_server = MCPServerStreamableHttp(
    params={"url": "http://127.0.0.1:8000/mcp"},
    cache_tools_list=True,
    name="InvoiceSQLiteMCP",
)

invoice_agent = Agent(
    name="InvoiceAssistant",
    instructions=(
        "Lookup invoices for a given customer name using MCP tools. "
...
async def test_queries():
    ...
    with trace("FitLife Assistant Test"):
      async with MCPServerManager([invoice_mcp_server], strict=True) as mcp_manager:
          invoice_agent.mcp_servers = mcp_manager.active_servers
